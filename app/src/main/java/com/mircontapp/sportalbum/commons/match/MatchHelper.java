...
import com.mirco.sportalbum.models.PlayerStatsModel;
import com.mirco.sportalbum.models.TiratoreModel;
import com.mirco.sportalbum.ui.tournaments.match.MatchManager;
...
public class MatchHelper {
    ...
    public static AlbumApplication app = AlbumApplication.getInstance();

    public static MatchModel centrocampo(MatchManager matchManager) {

        MatchModel matchModel = matchManager.getMatchModel();

        String protagonistaA = "";
        double cenA = -1d, pot = 0d, fix = 0d,  dado = 0d;

        //centrocampo azione offensiva
        for (PlayerModel attacker : matchManager.getAttackers()) {
            double partecipa = Math.random()*100d;
            if (partecipa < matchModel.getMinuto()/5d - attacker.getPlayerStatsModel().getFis()/10d) {
                partecipa = 0d;
            }
            if (partecipa > attacker.getRoleLineUp().getPartCen() && !attacker.isEspuslo()) {
                PlayerStatsModel attStats = attacker.getPlayerStatsModel();
                pot = attStats.getTec()/2 + attStats.getDri()/4 + attStats.getVel()/4;
                fix = matchManager.getLegend() ? attacker.getValueLegend() : attacker.getValue();
        ...
        if (cenA < 0) {
            double pl = Math.random()*10.9;
            PlayerModel attacker = matchManager.getAttackers().get((int) pl);
            PlayerStatsModel attStats = attacker.getPlayerStatsModel();
            pot = attStats.getTec()/2 + attStats.getDri()/4 + attStats.getVel()/4;
            dado = fix/2 + (Math.random()*pot/2);
            cenA=dado;
        ...
        String protagonistaD = "";
        double cenD=-1;
        for (PlayerModel defender : matchManager.getDefenders()) {
            double partecipa = Math.random()*100d;
            ...
            if (partecipa > defender.getRoleLineUp().getPartCen() && !defender.isEspuslo()) {
                PlayerStatsModel difStats = defender.getPlayerStatsModel();
                pot = difStats.getFis()/2 + difStats.getBal()/4 + difStats.getVel()/4;
                fix = matchManager.getLegend() ? defender.getValueLegend() : defender.getValue();
                dado = fix/2 + (Math.random()*pot/2);
                if (dado>cenD) {
                    cenD=dado;
        ...
        if (cenD < 0) {
            double pl = Math.random()*10.9;
            PlayerModel defender = matchManager.getDefenders().get((int) pl);
            PlayerStatsModel difStats = defender.getPlayerStatsModel();
            pot = difStats.getFis()/2 + difStats.getBal()/4 + difStats.getVel()/4;
            fix = matchManager.getLegend() ? defender.getValueLegend() : defender.getValue();
            dado = fix/2 + (Math.random()*pot/2);
        ...
        if (diff<0) {
            ...
            matchModel.setAzione((int) cenA);
            if (diff > -8) {
                messaggio = String.format(context.getString(R.string.telecronacaCen1), protagonistaA);
            } else if (diff > -15) {
                messaggio = String.format(context.getString(R.string.telecronacaCen2), protagonistaA);
            } else  {
                messaggio = String.format(context.getString(R.string.telecronacaCen3), protagonistaA);
            }
        //calcio di punizione
            ...
            matchModel.setAzione((int) cenA);
            messaggio = messaggio + " " + String.format(context.getString(R.string.telecronacaPun), protagonistaD);
        //vince la squadra difendente
            ...
            matchModel.setAzione((int) cenA);
            matchModel.setPossesso(matchModel.getPossesso() == Enums.Possesso.HOME ? Enums.Possesso.AWAY : Enums.Possesso.HOME);
            if (diff > 15) {
                messaggio = String.format(context.getString(R.string.telecronacaBal3), protagonistaD);
            } else if (diff > 8) {
                messaggio = String.format(context.getString(R.string.telecronacaBal2), protagonistaD);
            } else {
                messaggio = String.format(context.getString(R.string.telecronacaBal1), protagonistaD);
            }
        }

        matchModel.setStato(matchModel.getPossesso().ordinal());

        matchModel.setMessaggio(messaggio != null ? messaggio : protagonistaA);

        return matchModel;
    }


    public static MatchModel attacco(MatchManager matchManager) {
        MatchModel matchModel = matchManager.getMatchModel();

        String protagonistaA = "";
        double attA = -1d, pot = 0d, fix = 0d, dado = 0d;
        ...
        for (PlayerModel attacker : matchManager.getAttackers()) {
            ...
            if (partecipa > attacker.getRoleLineUp().getPartAtt() && !attacker.isEspuslo()) {
                ...
                PlayerStatsModel attStats = attacker.getPlayerStatsModel();
                pot = attStats.getAtt()/4 + attStats.getDri()/4 + attStats.getTec() / 4  + attStats.getVel()/4;
                fix = matchManager.getLegend() ? attacker.getValueLegend() : attacker.getValue();
        ...
        for (PlayerModel defender : matchManager.getDefenders()) {
            double partecipa = Math.random()*100d;
            if (partecipa < matchModel.getMinuto()/10d) {
                partecipa = 0d;
            }
            if (partecipa > defender.getRoleLineUp().getPartCen() && !defender.isEspuslo()) {
                PlayerStatsModel difStats = defender.getPlayerStatsModel();
                pot = difStats.getDif()/4 + difStats.getBal()/4 + difStats.getFis()/4 + difStats.getVel()/4;
                fix = matchManager.getLegend() ? defender.getValueLegend() : defender.getValue();
                dado = fix/4 + (Math.random()*pot/4*3);
                if (dado>difD) {
                    difD=dado;
        ...
        if (diff<0) {
            ...
            matchModel.setEvento(Enums.Evento.NONE);
            matchModel.setProtagonista(protagonistaA);
            matchModel.setAzione((int) attA);
            if (diff < 10) {
                messaggio = String.format(context.getString(R.string.telecronacaAtt1), protagonistaA);
            } else {
                messaggio = String.format(context.getString(R.string.telecronacaAtt2), protagonistaA);
            }
        //punizione
        ...
        //calcio di rigore
            ...
            matchModel.setPossesso(matchModel.getPossesso() == Enums.Possesso.HOME ? Enums.Possesso.AWAY : Enums.Possesso.HOME);
            if (diff < 5) {
                messaggio = String.format(context.getString(R.string.telecronacaDif1), protagonistaD);
            } else if (diff < 10) {
                messaggio = String.format(context.getString(R.string.telecronacaDif2), protagonistaD);
            } else {
                messaggio = String.format(context.getString(R.string.telecronacaDif3), protagonistaD);
            }
        }
    ...
    public static MatchModel finalizzazione(MatchManager matchManager) {
        MatchModel matchModel = matchManager.getMatchModel();

        String protagonistaA = "";
        PlayerModel goleador = null;
        double finA = -1d, pot = 0d, fix = 0d,  dado = 0d;
        ...
        double difPower = 1d, part = 0d;
        for (PlayerModel defender : matchManager.getDefenders()) {
            double partecipa = Math.random()*100d;
            if (defender.getRole() != Enums.Role.PT && partecipa > defender.getRoleLineUp().getPartCen() && !defender.isEspuslo()) {
                PlayerStatsModel difStats = defender.getPlayerStatsModel();
                pot = difStats.getDif()/4 + difStats.getBal()/4 + difStats.getFis()/4 + difStats.getVel()/4;
        ...
        PlayerModel portiere=matchManager.getDefenders().get(0);
        pot=portiere.getPlayerStatsModel().getPor()/2 + portiere.getPlayerStatsModel().getBal()/4 + portiere.getPlayerStatsModel().getDif()/4;
        fix = matchManager.getLegend() ? portiere.getValueLegend() : portiere.getValue();
        double parata = fix/2 + (Math.random()*pot/2) + difPower;

        Context context = app.getBaseContext();
        String messaggio = "";
        double diff=parata-finA;
        matchModel.setStato(matchModel.getPossesso().ordinal());
        ...
        if (diff<0) {
            ...
            if (goleador != null && matchModel.getMarcatori() != null) {
                MarcatoreModel marcatoreModel = new MarcatoreModel(goleador);
            ...
            if (diff<0 && diff>=-4) {
                messaggio = String.format(context.getString(R.string.telecronacaGol1), goleador.getName());
            } else if (diff<-4 && diff>=-8) {
                messaggio = String.format(context.getString(R.string.telecronacaGol2), goleador.getName());
            } else {
                messaggio = String.format(context.getString(R.string.telecronacaGol3), goleador.getName());
            }
            matchModel.setPossesso(matchModel.getPossesso() == Enums.Possesso.HOME ? Enums.Possesso.AWAY : Enums.Possesso.HOME);
            ...
            matchModel.setProtagonista(protagonistaA);
            matchModel.setCoprotagonista(portiere.getName());
            matchModel.setAzione((int) finA);
            messaggio = messaggio + " " + String.format(context.getString(R.string.telecronacaRig), goleador.getName());
        //vince la squadra difendente
        } else {
            matchModel.setFase(Enums.Fase.CENTROCAMPO);
            matchModel.setEvento(Enums.Evento.NONE);
            ...
            matchModel.setAzione((int) finA);
            if (diff>=1 && diff<2) {
                messaggio = String.format(context.getString(R.string.telecronacaPal), goleador.getName());
            } else if (diff>=2 && diff<3) {
                messaggio = String.format(context.getString(R.string.telecronacaTra), goleador.getName());
            } else if (diff>=3 && diff<5) {
                messaggio = String.format(context.getString(R.string.telecronacaPar1), goleador.getName(), portiere.getName());
            } else if (diff>=5 && diff<7) {
                messaggio = String.format(context.getString(R.string.telecronacaPar2), goleador.getName(), portiere.getName());
            } else if (diff>=7 && diff<9) {
                messaggio = String.format(context.getString(R.string.telecronacaOut1), goleador.getName());
            } else {
                messaggio = String.format(context.getString(R.string.telecronacaOut2), goleador.getName());
            }
            matchModel.setPossesso(matchModel.getPossesso() == Enums.Possesso.HOME ? Enums.Possesso.AWAY : Enums.Possesso.HOME);
    ...
    public static MatchModel punizione(MatchManager matchManager) {
        MatchModel matchModel = matchManager.getMatchModel();

        List<PlayerModel> tiratori = new ArrayList<>(matchManager.getAttackers());
        PlayerModel tiratore1 = tiratori.remove(findTiratore(tiratori));
        PlayerModel tiratore2 = tiratori.remove(findTiratore(tiratori));

        double soglia = tiratore1.getPlayerStatsModel().getPor() * 2d;
        double tot = soglia + tiratore2.getPlayerStatsModel().getRig();
        double dado = Math.random()*tot;

        PlayerModel tiratore = dado < soglia ? tiratore1 : tiratore2;

        double finA = -1d, pot = 0d, fix = 0d;

        if (matchManager.getMatchModel().getAzione() > 70) {
            matchModel = punizioneDiretta(matchManager, tiratore);
        } else {
            matchModel = punizioneGiocata(matchManager, tiratore);
        }

        return matchModel;
    }

    private static MatchModel punizioneDiretta(MatchManager matchManager, PlayerModel tiratore) {
        MatchModel matchModel = matchManager.getMatchModel();
        ...
        fix = matchManager.getLegend() ? tiratore.getValueLegend() : tiratore.getValue();
        finA = fix/2 + (Math.random()*pot/2);

        PlayerModel portiere=matchManager.getDefenders().get(0);
        pot=portiere.getPlayerStatsModel().getPor()/2 + portiere.getPlayerStatsModel().getBal()/4 + portiere.getPlayerStatsModel().getDif()/4;
        fix = matchManager.getLegend() ? portiere.getValueLegend() : portiere.getValue();
        double parata = fix/4*3 + (Math.random()*pot)/4*3;
        ...
        if (diff<0) {
            ...
            if (tiratore != null && matchModel.getMarcatori() != null) {
                MarcatoreModel marcatoreModel = new MarcatoreModel(tiratore);
                marcatoreModel.setMinute(matchModel.getMinuto());
                matchModel.getMarcatori().add(marcatoreModel);
            }
            matchModel.setProtagonista(protagonistaA);
            matchModel.setCoprotagonista(portiere.getName());
            if (diff<0 && diff>=-4) {
                messaggio = String.format(context.getString(R.string.telecronacaPun1), tiratore.getName());
            } else {
                messaggio = String.format(context.getString(R.string.telecronacaPun2), tiratore.getName());
            }
        //vince la squadra difendente
        } else {
            matchModel.setFase(Enums.Fase.CENTROCAMPO);
            ...
            matchModel.setAzione((int) finA);
            if (diff>=2 && diff<4) {
                messaggio = String.format(context.getString(R.string.telecronacaPar1), tiratore.getName(), portiere.getName());
            } else {
                messaggio = String.format(context.getString(R.string.telecronacaPar2), tiratore.getName(), portiere.getName());
            }
        }
        matchModel.setMessaggio(messaggio);
        matchModel.setPossesso(matchModel.getPossesso() == Enums.Possesso.HOME ? Enums.Possesso.AWAY : Enums.Possesso.HOME);
    ...
    private static MatchModel punizioneGiocata(MatchManager matchManager, PlayerModel tiratore) {
        MatchModel matchModel = matchManager.getMatchModel();
        ...
        pot = tiratore.getPlayerStatsModel().getRig()/4*3 + tiratore.getPlayerStatsModel().getTec()/4;
        fix = matchManager.getLegend() ? tiratore.getValueLegend() : tiratore.getValue();
        finA = fix/2 + (Math.random()*pot/2);
        ...
        if (diff<0) {
            ...
            for (PlayerModel attacker : matchManager.getAttackers()) {
                double partecipa = attacker.getRoleLineUp().getPartfin()/ 2 + Math.random()*50d;
                if (partecipa > attacker.getRoleLineUp().getPartfin() && !attacker.isEspuslo()) {
                    PlayerStatsModel attStats = attacker.getPlayerStatsModel();
                    pot=attStats.getFin()/2 + attStats.getAtt()/4 + attStats.getVel()/4;
                    fix = matchManager.getLegend() ? attacker.getValueLegend() : attacker.getValue();
            ...
            pot=portiere.getPlayerStatsModel().getPor()/2 + portiere.getPlayerStatsModel().getBal()/4 + portiere.getPlayerStatsModel().getDif()/4;
            fix = matchManager.getLegend() ? portiere.getValueLegend() : portiere.getValue();
            double parata = fix/8*7 + (Math.random()*pot/8);
    ...
    public static MatchModel rigore(MatchManager matchManager) {
        MatchModel matchModel = matchManager.getMatchModel();

        List<PlayerModel> tiratori = new ArrayList<>(matchManager.getAttackers());
        PlayerModel tiratore = tiratori.remove(findTiratore(tiratori));

        double finA = -1d, pot = 0d, fix = 0d;

        PlayerStatsModel attStats = tiratore.getPlayerStatsModel();
        ...
        pot=portiere.getPlayerStatsModel().getPor()/2 + portiere.getPlayerStatsModel().getBal()/4 + portiere.getPlayerStatsModel().getDif()/4;
        fix = matchManager.getLegend() ? portiere.getValueLegend() : portiere.getValue();
        double parata = fix/2 + (Math.random()*pot/2);
        ...
        if (diff<0) {
            ...
            matchModel.setCoprotagonista(portiere.getName());
            if (diff<0 && diff>=-4) {
                messaggio = String.format(context.getString(R.string.telecronacaRig1), tiratore.getName());
            } else  {
                messaggio = String.format(context.getString(R.string.telecronacaRig2), tiratore.getName());
            }
        } else {
            matchModel.setFase(Enums.Fase.CENTROCAMPO);
            matchModel.setEvento(Enums.Evento.NONE);
            ...
            matchModel.setAzione((int) finA);
            if (diff>=0 && diff<1) {
                messaggio = String.format(context.getString(R.string.telecronacaPal), tiratore.getName());
            } else if (diff>=1 && diff<2) {
                messaggio = String.format(context.getString(R.string.telecronacaTra), tiratore.getName());
            } else if (diff>=2 && diff<5) {
                messaggio = String.format(context.getString(R.string.telecronacaParig), portiere.getName());
            } else if (diff>=5 && diff<7) {
                messaggio = String.format(context.getString(R.string.telecronacaPar1), tiratore.getName(), portiere.getName());
            } else if (diff>=7 && diff<9) {
                messaggio = String.format(context.getString(R.string.telecronacaOut2), tiratore.getName());
            }
        }
        matchModel.setMessaggio(messaggio);
        matchModel.setPossesso(matchModel.getPossesso() == Enums.Possesso.HOME ? Enums.Possesso.AWAY : Enums.Possesso.HOME);
    ...
    public static MatchModel rigoreDiretto(MatchManager matchManager, TiratoreModel tiratore, PlayerModel portiere) {
        MatchModel matchModel = matchManager.getMatchModel();

        double finA = -1d, pot = 0d, fix = 0d;

        PlayerStatsModel attStats = tiratore.getPlayerModel().getPlayerStatsModel();
        pot=attStats.getRig()/4*3 + attStats.getFin()/4;
        fix = matchManager.getLegend() ? tiratore.getPlayerModel().getValueLegend() : tiratore.getPlayerModel().getValue();
        finA = fix/2 + (Math.random()*pot);
        ...
        if (diff<0) {
            ...
            if (diff<0 && diff>=-4) {
                messaggio = String.format(context.getString(R.string.telecronacaRig1), tiratore.getPlayerModel().getName());
            } else  {
                messaggio = String.format(context.getString(R.string.telecronacaRig2), tiratore.getPlayerModel().getName());
            }
        } else {
            matchModel.setAzione((int) finA);
            if (diff>=0 && diff<1) {
                messaggio = String.format(context.getString(R.string.telecronacaPal), tiratore.getPlayerModel().getName());
            } else if (diff>=1 && diff<2) {
                messaggio = String.format(context.getString(R.string.telecronacaTra), tiratore.getPlayerModel().getName());
            } else if (diff>=2 && diff<5) {
                messaggio = String.format(context.getString(R.string.telecronacaParig), portiere.getName());
            } else if (diff>=5 && diff<7) {
                messaggio = String.format(context.getString(R.string.telecronacaPar1), tiratore.getPlayerModel().getName(), portiere.getName());
            } else {
                messaggio = String.format(context.getString(R.string.telecronacaOut2), tiratore.getPlayerModel().getName());
            }
        }
    ...
    public static int findTiratore(List<PlayerModel> playerModels) {
        int index = 0;
        double mRig = 0d;
        for(int i = 0; i<playerModels.size(); i++) {
            PlayerModel p = playerModels.get(i);
